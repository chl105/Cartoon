 ...
 * Date: 2017/3/11
 */
public class CartoonClassifyHttp extends BaseHttp {

    public CartoonClassifyHttp(Map<String, String> params) {
        super(params);
    }

    @Override
    protected Call<ResponseBody> createServer() {
        ClassifyServer server = new Retrofit.Builder()
                .baseUrl(Consts.CARTOON_URL)
                .addConverterFactory(GsonConverterFactory.create())
                .build()
                .create(ClassifyServer.class);
        return server.request(params);
    }

    interface ClassifyServer {

        @Headers("User-Agent: kidstone.cn/2.7.2/30/Android/4.4.4/MI-4C/2e59c41af4797b51/YingYongBao")
        @GET("dacu_app/get_tag_book.php")
